...
## 문서 목적
이 문서는 대화 컨텍스트가 초기화된 새 세션에서도 현재 저장소 상태를 빠르게 복구할 수 있도록 만든 상세 인수인계 문서다. `docs/project-status.md`가 현재 상태 요약이라면, 이 문서는 왜 이런 구조가 되었는지와 다음 작업 진입 절차까지 포함한다.
...
- `mobile-flutter/`
  ...
  - `android/`, `ios/`: Flutter 기본 플랫폼 프로젝트
  - `test/widget_test.dart`

### 문서
- `AGENTS.md`: 저장소 운영 규칙
- `docs/planning.md`: 제품 비전과 초기 계획
- `docs/platform-direction.md`: 플랫폼 방향
- `docs/flutter-architecture.md`: Flutter 기준 아키텍처
- `docs/api-spec.md`: 백엔드-앱 공용 API 계약 초안
- `docs/project-status.md`: 현재 상태 요약
- `docs/handoff/2026-03-19.md`: 일자별 작업 기록
- `docs/session-continuity.md`: 현재 문서
...
```bash
cd /home/msyeo/mongtorydiary/mobile-flutter
HOME=/home/msyeo/mongtorydiary/.tmp_flutter_home /home/msyeo/mongtorydiary/.tooling/flutter-sdk/bin/flutter pub get
HOME=/home/msyeo/mongtorydiary/.tmp_flutter_home /home/msyeo/mongtorydiary/.tooling/flutter-sdk/bin/flutter analyze
HOME=/home/msyeo/mongtorydiary/.tmp_flutter_home /home/msyeo/mongtorydiary/.tooling/flutter-sdk/bin/flutter test
```
...
```bash
cd /home/msyeo/mongtorydiary
JAVA_HOME=/usr/lib/jvm/java-17-openjdk-amd64 bash ./mvnw -Dmaven.repo.local=/home/msyeo/mongtorydiary/.m2 test
```
...
## 로컬 환경 및 도구 위치
- 저장소 루트: `/home/msyeo/mongtorydiary`
- 현재 주요 작업 브랜치: `feature/basic`
- Flutter SDK: `/home/msyeo/mongtorydiary/.tooling/flutter-sdk`
- Flutter 실행 파일: `/home/msyeo/mongtorydiary/.tooling/flutter-sdk/bin/flutter`
- Maven 실행 기준 Java:
...
현재 눈에 띄는 특징:
- `mvnw`, `pom.xml`, `src/main/resources/application.properties`는 이미 수정 상태
- `docs/`, `mobile-flutter/`, 백엔드 API 골격 파일들은 아직 추적 전이거나 새로 추가된 상태
- `.m2/`, `.tmp_flutter_home/`, `.tooling/` 같은 로컬 산출물이 존재

## 새 세션 시작 체크리스트
1. `AGENTS.md`를 먼저 읽어 작업 규칙을 확인한다.
2. `docs/project-status.md`로 전체 상태를 빠르게 확인한다.
3. `docs/session-continuity.md`로 전환 배경과 현재 구조를 복구한다.
4. `docs/handoff/2026-03-19.md`에서 직전 작업 흐름을 확인한다.
5. `git status --short`로 기존 변경 파일과 신규 파일 상태를 점검한다.
6. 작업 종료 직후 `docs/project-status.md`와 `docs/handoff/YYYY-MM-DD.md`를 바로 갱신한다.

## 다음 우선순위
### 1순위
- Flutter 원격 API 모드로 실제 백엔드 연결 검증
- 인증, 일기 목록, 캘린더 조회의 성공/실패 플로우를 화면에 더 구체적으로 반영
- 실제 원격 모드 기준으로 provider와 세션 상태를 다듬기
...
## 새 세션에서 바로 쓸 수 있는 요약
이 저장소는 더 이상 React 웹이 메인이 아니다. 현재 유효한 메인 방향은 `Spring Boot API + Flutter 앱 + 추후 별도 웹 + Android/iOS 위젯`이다. Flutter 앱은 이미 실제 프로젝트로 초기화되어 있고 Riverpod, 도메인 모델, DTO, 매퍼, mock/remote datasource와 repository까지 연결된 상태다. 백엔드는 실제 API 계약을 따라가는 컨트롤러/서비스/DTO 구조가 추가되었고, Diary/Calendar와 Auth는 SQLite 영속화 뼈대와 임시 access token 기반 사용자 식별까지 들어간 상태다. 백엔드 Maven 테스트와 Flutter analyze/test는 모두 통과했다. 다음 작업은 실제 원격 연동 검증이나, JWT 정책 고도화 쪽이 가장 자연스럽다.
