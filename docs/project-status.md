...
GitHub 원격 푸시는 현재 HTTPS 인증 미설정 상태이며, 로컬 환경에서는 `gh` 설치가 끝났으므로 `gh auth login` 또는 SSH 키 설정 후 진행하면 된다.
2026-03-20 기준 `gh auth login`, `gh auth setup-git`을 통해 HTTPS 인증 연동이 완료되었고 `feature/basic` 브랜치 푸시도 성공했다.
