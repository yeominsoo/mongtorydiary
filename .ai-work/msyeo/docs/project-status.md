...
2026-04-28에는 `.ai-work/msyeo/docs/delivery-roadmap.md`를 추가해 MVP 완료까지의 단계적 업무계획을 정리했다. BE `REQ-20260428-22` API 검증 오류 계약 보강은 구현과 Maven 검증을 완료했고, QA `REQ-20260428-29` 실제 서버 회귀 검증도 통과했다. QA `REQ-20260428-24` 일기 CRUD 회귀 시나리오 및 자동화 확장은 완료되었고, 실제 서버 remote CRUD API도 로그인 -> 생성 -> 상세 -> 수정 -> 목록 반영 -> 삭제 -> 목록 제외 -> invalid token 401 순서로 확인했다. PM `REQ-20260428-25`는 `.ai-work/msyeo/docs/widget-deeplink-plan.md` 작성으로 완료됐다. FE `REQ-20260428-23`은 `REQ-20260428-09` Flutter 일기 생성/수정/삭제 구현 결과에 흡수되어 보류 처리했다. 후속 사용성 설계 요청은 FE `REQ-20260428-26`과 BE `REQ-20260428-27` 모두 완료됐다. BE는 `REQ-20260428-27`에서 오늘/위젯 요약 API를 1순위 구현 후보로 선정했다. PM은 `REQ-20260428-31`로 동일 날짜 다건 일기 UX 기준을 최신 1건 직행이 아니라 날짜별 목록 또는 bottom sheet 선택 방식으로 결정했다. FE는 `REQ-20260428-28` 기준으로 캘린더 날짜 탭 시 해당 날짜 일기 목록 bottom sheet를 보여주고, 일기 선택 시 상세 화면으로 이동하며, 기록이 없는 날짜 또는 추가 작성은 선택 날짜가 기본값인 작성 화면으로 이어지도록 구현했다. PM은 Flutter `analyze`와 `test` 재실행 통과를 확인했고, QA `REQ-20260428-30`은 하네스 기준으로 기존 날짜 상세 진입과 빈 날짜 작성 기본값을 검증해 완료했다. BE `REQ-20260428-27`은 사용성 개선 API 후보 설계를 완료했고 오늘/위젯 요약 API를 1순위 구현 후보로 제안했다. PM은 기획 개선 요청 `REQ-20260428-32`로 FE `REQ-20260428-33`, BE `REQ-20260428-34`, QA `REQ-20260428-35`, PM `REQ-20260428-36`을 추가 배정했다.
...
## 다음 우선 작업
1. BE: `REQ-20260428-34` 오늘/위젯 요약 API 1차 구현
2. FE: `REQ-20260428-33` 일기 작성 UX 정식 개선과 진행 중인 `FE-IMPROVE-20260428-04` 범위 정리
3. QA: `REQ-20260428-35` MVP 릴리스 회귀 체크리스트와 신규 개선 검증 계획 작성
4. PM: BE `REQ-20260428-34` 결과 확인 후 `REQ-20260428-36` 위젯/딥링크 구현 요청 분해
5. PM: 완료 요청의 responses/handoff/검증 결과를 보고 진행 중 FE 코드와 분리해 커밋 지속
6. FE/QA: 캘린더 날짜 탭 remote 런타임 검증 범위가 필요하면 별도 요청으로 분리
