...
- `delivery-roadmap.md`: 목표 달성을 위한 단일 세션 단계적 업무계획
- `single-session-worklist.md`: 역할 분배 철회 후 남은 작업의 중앙 목록
...
- `collaboration/master-flow.md`: 단일 세션 운영 마스터 플로우
- `collaboration/requests.md`: 단일 세션 공통 요청 인덱스
- `collaboration/status.md`: 단일 세션 진행 상태 인덱스
- `collaboration/responses.md`: 단일 세션 요청별 응답 인덱스
- `collaboration/roles/README.md`: 과거 역할별 문서 보관 안내
- `collaboration/roles/{pm|qa|fe|be}/`: 과거 역할별 요청, 수신함, 현황, 응답 이력
- `collaboration/roles/{pm|qa|fe|be}/handoff/`: 과거 역할별 작업 블록 종료 기록
- `collaboration/roles/pm/monitoring.md`: 과거 PM 모니터링 절차 참고 문서
...
- 새 요청과 진행 상태는 `single-session-worklist.md`와 공통 `collaboration/requests.md`, `status.md`, `responses.md`에 남긴다.
- 역할별 디렉토리는 과거 이력 확인용이며 신규 작업 기록 대상이 아니다.
- 작업 블록 종료 시 전체 handoff인 `.ai-work/msyeo/docs/handoff/YYYY-MM-DD.md`를 작성한다.
- 커밋 메시지는 한글로 작성한다.
